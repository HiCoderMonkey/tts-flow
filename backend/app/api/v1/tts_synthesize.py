...
async def tts_synthesize(req: TTSRequest):
    ...
    if not flow:
        def err():
            yield f"data: {json.dumps({'data': 'flow not found', 'type': 'error', 'end': True})}\n\n"
        return StreamingResponse(err())
    voice_id = flow.voiceId
    if not voice_id:
        def err():
            yield f"data: {json.dumps({'data': 'voiceId not set', 'type': 'error', 'end': True})}\n\n"
        return StreamingResponse(err())
    voice = await TTSVoice.get(voice_id)
    if not voice:
        def err():
            yield f"data: {json.dumps({'data': 'voice not found', 'type': 'error', 'end': True})}\n\n"
        return StreamingResponse(err())
    platform = await TTSPlatform.get(voice.platform_id)
    if not platform:
        def err():
            yield f"data: {json.dumps({'data': 'platform not found', 'type': 'error', 'end': True})}\n\n"
        return StreamingResponse(err())
    if platform.type == "volcano":
        wav_dir = os.path.join(AppConstants.STATIC_DIR, "tts_wav", req.flow_id)
        os.makedirs(wav_dir, exist_ok=True)
        wav_path = os.path.join(wav_dir, f"{req.node_id}.wav")
        def event_generator():
            for chunk in tts_volcano_stream(
                ...
                platform=platform,
                wav_path=wav_path,
                relative_path=f"tts_wav/{req.flow_id}/{req.node_id}.wav"
            ):
                yield f"data: {json.dumps(chunk)}\n\n"
        return StreamingResponse(event_generator())
    else:
        def err():
            yield f"data: {json.dumps({'data': 'not implemented', 'type': 'error', 'end': True})}\n\n"
        return StreamingResponse(err()) 