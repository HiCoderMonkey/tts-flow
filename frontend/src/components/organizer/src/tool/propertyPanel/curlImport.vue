<template>
  <div class="curl-import">
    ...
    <el-dialog
      ...
      destroy-on-close
      title="cURL语句导入"
      custom-class="ds-dialog__import"
      append-to-body
    >
      <div>
        <el-input type="textarea" v-model="dialogInfo.value" :autosize="{ minRows: 8 }" />
      </div>
...
export default {
  ...
  methods: {
    ...
    transJsonToTable(params) {
      ...
      for (const key in params) {
        if (Object.prototype.hasOwnProperty.call(params, key)) {
          ...
          const paramExample =
            fieldTypeName === 'json' || fieldTypeName === 'list'
    ...
    getPropertyType(property) {
      const fieldTypeName = {}.toString.call(property).slice(8, -1).toLowerCase()
      return {
