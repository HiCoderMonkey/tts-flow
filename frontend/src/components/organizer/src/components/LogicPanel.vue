...
export default {
  ...
  methods: {
    ...
    initCanvas() {
      ...
      if (!graphData || graphData.nodes.length === 0) this.initNode = this.graph.addInitNode()
      this.graph.initView()
    ...
    initEvents() {
      // Context 事件监听
      // 点击组件时高亮对应节点
      this.context.eventCenter.on(EDITOR_EVENT.CANVAS_MODEL_CLICKED, (model) => {
        this.graph.selectNodesByModel(model)
      })
      this.context.eventCenter.on(EDITOR_EVENT.CANVAS_MODEL_DELETED, (model) => {
      ...
      this.lf.on('node:delete-node', (model) => {
        ...
        if (data.type === 'common-node' && data.properties && data.properties.ds) {
      ...
      this.lf.on('node:delete', ({ data }) => {
        ...
        } else if (data.properties && data.properties.componentName === 'pageInit') {
          const hasInit = nodes.find(
            (item) => item.properties && item.properties.componentName === 'pageInit'
          )
          if (!hasInit) {
    ...
    setGraphDataToContext() {
      ...
      this.$emit('update-logic-list', [graphData])
    },
    /**
     * 获取节点的outgoing
     * outgoing: [
