...
import { ElTable, ElTableColumn, ElButton, ElInput, ElDialog, ElForm, ElFormItem, ElMessage, ElMessageBox } from 'element-plus'
    ...
    >
      ...
      <ElTableColumn prop="flow_config" label="Flow配置" min-width="150">
        <template #default="{ row }">
          <div class="text-sm text-gray-600">
            {{ getNodeCount(row.flow_config) }} 个节点
          </div>
        </template>
      </ElTableColumn>
      ...
      <ElTableColumn label="操作" width="200" fixed="right">
        <template #default="{ row }">
          ...
          <ElButton type="info" size="small" @click="goToCanvas(row)">
            配置
          </ElButton>
        </template>
      </ElTableColumn>
